...
export function Experience() {
  const experienceItems = [
    {
      icon: <Tag className="h-10 w-10 md:h-12 md:w-12 text-navy" />,
    ...
    {
      icon: <Battery className="h-10 w-10 md:h-12 md:w-12 text-navy" />,
    ...
    {
      icon: <Wifi className="h-10 w-10 md:h-12 md:w-12 text-navy" />,
  ...
  return (
    <section id="experience" className="py-8 md:py-24 bg-white">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center text-center space-y-3 mb-8">
          <h2 className="text-2xl md:text-5xl font-bold text-navy">
            Nous sommes prêts à partager notre maîtrise technique
          </h2>
          <p className="text-base md:text-xl text-navy/70 max-w-3xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {experienceItems.map((item, index) => (
            <div
              key={index}
              className="bg-gray-50 rounded-lg p-4 md:p-8 flex flex-col items-center text-center space-y-3"
            >
              <div className="mb-2">{item.icon}</div>
              <h3 className="text-lg md:text-xl font-semibold text-navy">{item.title}</h3>
              <p className="text-navy/70 text-sm md:text-base">{item.description}</p>
            </div>
          ))}
        </div>

        <div className="bg-orange-500 rounded-lg p-6 md:p-12">
          <div className="grid md:grid-cols-2 gap-6 md:gap-8 items-center">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              <div className="bg-white/10 p-4 md:p-6 rounded-lg text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-2">1200+</div>
                <div className="text-white/90 text-sm md:text-base">Magasins installés</div>
              </div>
              <div className="bg-white/10 p-4 md:p-6 rounded-lg text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-2">13M+</div>
                <div className="text-white/90 text-sm md:text-base">Étiquettes installées</div>
              </div>
              <div className="bg-white/10 p-4 md:p-6 rounded-lg text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-2">15+</div>
                <div className="text-white/90 text-sm md:text-base">Ans d'expérience</div>
              </div>
              <div className="bg-white/10 p-4 md:p-6 rounded-lg text-center">
                <div className="text-2xl md:text-3xl font-bold text-white mb-2">30K+</div>
                <div className="text-white/90 text-sm md:text-base">Professionnels formés</div>
              </div>
            </div>
          </div>
        </div>
